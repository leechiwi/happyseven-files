...
@Slf4j
public class EpubConvert extends AbstractApHtmlConvert {
    ...
    protected boolean doConvert(Object in, String out, HtmlConvertType htmlConvertType, OptionResult optionResult) {
        try {
            ...
            log.error("epub converted to " + htmlConvertType.getExt() + " by aspose-html error", e);
            return false;
        }
        return true;
    }
    ...
    public boolean convertTo(PdfSaveOptions pdfSaveOptions) {
        ...
        Converter.convertEPUB(this.input, pdfSaveOptions, this.out);
        return true;
    }

    @Override
    public boolean convertTo(ImageSaveOptions imageSaveOptions) {
        Converter.convertEPUB(this.input, imageSaveOptions, this.out);
        return true;
    }

    @Override
    public boolean convertTo(MHTMLSaveOptions MHTMLSaveOptions) {
        throw new HappysevenException("epub can not be converted to mhtml by aspose-html");
    }

    @Override
    public boolean convertTo(XpsSaveOptions xpsSaveOptions) {
        xpsSaveOptions.setBackgroundColor(com.aspose.html.drawing.Color.getCyan());
        Converter.convertEPUB(this.input, xpsSaveOptions, this.out);
        return false;
    }

    @Override
    public boolean convertTo(MarkdownSaveOptions markdownSaveOptions) {
        throw new HappysevenException("epub can not be converted to markdown by aspose-html");
    }

    @Override
    public boolean convertTo(HtmlConvertType fileConvertType) {
        throw new HappysevenException("epub can not be converted to html by aspose-html");
    }
}
